...
export async function recommend(req, res, next) {
  try {
    ...
    if (option === 'jeonse') {
      ...
      const jeonseData = housingData.filter(
        (data) =>
          data.jeonseDeposit >= min_price && data.jeonseDeposit <= max_price
      );
      const jeonseIds = jeonseData.map((data) => data.id);

      // id값을 비교해 최종 점수 필터링 및 정렬
      const filterTotalScore = totalData
        .filter((data) => jeonseIds.some((id) => id.equals(data.id)))
      ...
      // 예산 범위에 맞는 id값 배열로 추출
      const monthData = housingData.filter(
        (data) =>
          data.monthDeposit >= min_price &&
          data.monthDeposit <= max_price &&
          data.monthRent >= min_price_2 &&
          data.monthRent <= max_price_2
      );
      const monthIds = monthData.map((data) => data.id);

      // id값을 비교해 최종 점수 필터링 및 정렬
      const filterTotalScore = totalData
        .filter((data) => monthIds.some((id) => id.equals(data.id)))
        .sort((a, b) => b.totalScore - a.totalScore);
